...
# kontrola velikosti logu: toto omezeni jsem pridal az po testu ve virtualce.  
IFS=""
if [ $(echo $file | wc -l) -gt 14000 ] ; then echo ERROR: log file reached maximum count of lines. ; exit 6 ; fi
unset IFS
	...
	if [ ! $CMD ] ; then 
		...
		case $CMD in
			...
			hist-ord)
				...
				for tck in $listoftck ; do
					printf "$tck \t : "
					actual=$(printf "$filtered\n" | grep -c ";$tck;")
					if [ $WIDTH ] ; then 
						raw=0
						raw=$( echo "$frac * $actual - 0.5" | bc )
						count=$(printf "%.0f\n" $raw)
